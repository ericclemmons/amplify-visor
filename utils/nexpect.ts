...
function chain(context: Context): ExecutionContext {
  return {
    ...
    run: function (
      callback: (err: any, code?: number, signal?: string | number) => void
    ): ExecutionContext {
      ...
      const exitHandler = (code: number, signal: any) => {
        ...
        if (code !== 0) {
          if (code === EXIT_CODE_TIMEOUT) {
            const err = new Error(
              `Killed the process as no output receive for ${
                context.noOutputTimeout / 1000
              } Sec. The no output timeout is set to ${
                context.noOutputTimeout / 1000
              }`
            );
            ...
            //
            return onError(
              new Error("Command not found: " + context.command),
              false
            );
          }
          return onError(
      ...
      function onError(
        err: any,
        kill: boolean,
        errorCode: number = EXIT_CODE_GENERIC_ERROR
      ) {
      ...
      function evalContext(data: string, name?: string): void {
        ...
        if (currentFnName === "_expect") {
          ...
          return currentFn(data) === true
            ...
            : onError(createExpectationError(step.expectation, data), true);
      ...
      function onLine(data: string | Buffer) {
        ...
        if (
          process.env &&
          process.env.VERBOSE_LOGGING_DO_NOT_USE_OR_YOU_WILL_BE_FIRED
        ) {
          console.log(data);
        }
      ...
      function flushQueue() {
        const remainingQueue = context.queue.slice().map((item) => {
          const description = ["_sendline", "_send"].includes(item.name)
            ? `[${item.name}] **redacted**`
            : item.description;
          return {
            ...item,
