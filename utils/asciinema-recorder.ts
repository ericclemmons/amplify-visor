...
export class Recorder {
  ...
  write(data: string): void {
    ...
    throw new Error(
      "Can not write data. Program is either already executed or has not been run"
    );
  }
  ...
  removeOnExitHandlers(
    fn: (code: number, signal: string | number) => void
  ): boolean {
  ...
  getRecording(): string {
    return [
      JSON.stringify(this.recording.header),
      ...this.recording.frames.map((frame) => JSON.stringify(frame)),
    ].join("\n");
  }
  ...
  private renderPrompt(cwd: string, cmd?: string, args?: string[]) {
    const separator = "\u2b80";
    const basePrompt = `${chalk.bgBlack("user@host") + chalk.black(separator)}${
      chalk.bgBlue(cwd) + chalk.blue(separator)
    }`;
